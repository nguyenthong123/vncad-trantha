  ...
  for (let sIdx = 0; sIdx < segs.length; sIdx++) {
    ...
    if (orderedPts.length >= 3) {
      ...
      for (let i = 0; i < orderedPts.length; i++) {
        let p = orderedPts[i];
        if (cleanLoop.length === 0 || Math.hypot(p.x - cleanLoop[cleanLoop.length - 1].x, p.y - cleanLoop[cleanLoop.length - 1].y) > 2) {
          cleanLoop.push(p);
        }
      }
      ...
      if (cleanLoop.length >= 3) {
        ...
        if (cleanLoop.length >= 3 && polyArea(cleanLoop) > 100) {
...
function getHSegments(polyPts, y0, isMeter = false) {
  ...
  for (let k = 0; k < deduped.length - 1; k++) {
    ...
    if (inMid || (inAbove && inBelow) || inAbove || inBelow) {
      ...
      let insideCount = testPoints.filter(p => isPointInPoly(p, polyPts) || isPointInPoly({ x: p.x, y: y0 + 2 }, polyPts) || isPointInPoly({ x: p.x, y: y0 - 2 }, polyPts)).length;
      if (insideCount >= 2) {
        pairs.push([s0, s1]);
      }
    }
...
function getVSegments(polyPts, x0, isMeter = false) {
  ...
  for (let k = 0; k < deduped.length - 1; k++) {
    ...
    if (inMid || (inLeft && inRight) || inLeft || inRight) {
      ...
      let insideCount = testPoints.filter(p => isPointInPoly(p, polyPts) || isPointInPoly({ x: x0 + 2, y: p.y }, polyPts) || isPointInPoly({ x: x0 - 2, y: p.y }, polyPts)).length;
      if (insideCount >= 2) {
        pairs.push([s0, s1]);
      }
    }
...
function init3DIsoViewer(allRooms, initialIndex) {
  ...
  function drawConstructionScene(isBottomUp, showWallV, showHangers, showMain, showCross, showCross6, showTiles, showWalls) {
  ...
  function renderLoop() {
    ...
    drawConstructionScene(isBottomUp, showWallV, showHangers, showMain, showCross, showCross6, showTiles, showWalls);
