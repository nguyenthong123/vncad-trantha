...
// Hàm xử lý Enter / Space toàn cục cho AutoCAD & Plugin
function handleGlobalConfirm(e, isFromCli = false) {
  if (!isFromCli) {
  ...
  // 1. Generic Enter Key Handler cho plugin hoặc custom tool (qua toolHandlers)
  if (window.cadPluginHooks && window.cadPluginHooks.toolHandlers && window.cadPluginHooks.toolHandlers[currentTool]) {
    ...
    if (typeof th.onEnter === 'function') {
      ...
      try {
        ...
        console.error("Lỗi khi chạy onEnter của toolHandler:", err);
      }
    }
  }

  // 2. Các handler Enter từ Plugin nạp ngoài (qua enterHandlers array)
  if (window.cadPluginHooks && Array.isArray(window.cadPluginHooks.enterHandlers) && window.cadPluginHooks.enterHandlers.length > 0) {
    ...
    for (let fn of window.cadPluginHooks.enterHandlers) {
      ...
      } catch (err) { console.error("Lỗi khi chạy enterHandler:", err); }
    }
    if (anyHandled) {
      if (e && typeof e.preventDefault === 'function') e.preventDefault();
      return true;
...
window.handleGlobalConfirm = handleGlobalConfirm;
...
window.addEventListener('keydown', (e) => {
  ...
  if ((e.ctrlKey || e.metaKey) && (e.key === 's' || e.key === 'S')) {
    ...
    if (typeof quickSave === 'function') {
      ...
      localStorage.setItem('vinacad_drawing_quicksave', JSON.stringify(data));
      setInfo("💾 Đã lưu nhanh bản vẽ vào bộ nhớ trình duyệt.", 'success');
    }
    return;
  }

  // Ctrl+O / Cmd+O : Mở nhanh bản vẽ
  if ((e.ctrlKey || e.metaKey) && (e.key === 'o' || e.key === 'O')) {
    e.preventDefault();
    let fileInput = document.getElementById('file-input-cad');
  ...
  if ((e.ctrlKey || e.metaKey) && (e.key === 'n' || e.key === 'N')) {
    e.preventDefault();
    if (confirm("Bạn có chắc chắn muốn tạo bản vẽ mới? Mọi nét vẽ chưa lưu sẽ bị xóa.")) {
      clearCanvas();
      logToCliHistory('NEW: Đã tạo bản vẽ mới sạch sẽ.', 'prompt');
    }
    return;
  }
  ...
  // Phím Enter
  if (e.key === 'Enter') {
    if (document.activeElement !== cliInput && document.activeElement !== dynInput) {
      handleGlobalConfirm(e, false);
    }
    return;
  }
